...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect based on role: donor -> /donor, rescuer -> /rescuer, admin -> /admin
      if (role === 'donor') {
        navigate('/donor');
    ...
    <div className="min-h-screen bg-[#FAF9F6] text-[#1F2937] flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-stone-600">
          Or{' '}
          <Link to="/signup" className="font-medium text-[#1F7A4D] hover:underline">
        ...
        <div className="bg-white py-8 px-6 sm:px-8 rounded-2xl border border-stone-200/70 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-[#1F7A4D] hover:bg-[#18643e] text-white font-medium text-sm transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {loading ? 'Logging in...' : 'Log In'}
            </button>
          </form>

          {/* Helper hint for admin or testers */}
          <div className="mt-6 pt-5 border-t border-stone-100 text-center text-xs text-stone-500 space-y-1">
            <p className="font-medium text-stone-600">Sample Credentials:</p>
            <p>Admin: <code className="bg-stone-100 px-1 py-0.5 rounded text-stone-700">admin@foodrescue.org</code> / <code className="bg-stone-100 px-1 py-0.5 rounded text-stone-700">AdminPassword123!</code></p>
          </div>
        </div>
      </div>
    </div>
